...
describe('Github Api Test 4', () => {
  describe('testing POST and PATCH method', async () => {
    ...
    // Gets repository list and selects one of them randomly and checks if it is public
    it('Repo test', async () => {
      const user = await object.get('https://api.github.com/user');
      const repos = await object.get(user.data.repos_url);
      const repo = _.sample(repos.data);
      expect(repo.private).to.equal(false);
    });
    /* Gets repository, selects one of them randomly and creates a new issue using POST method
    ...
    Checks if teh tittle is the same and the body is correct */
